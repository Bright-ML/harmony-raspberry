...
make lib下添加单板文件夹，在109行左右。同时在源码目录/vendor/下添加对应文件夹，鸿蒙的部分代码未开源，这里要感谢韦东山老师提供的补丁文件。

```
else ifeq ($(LOSCFG_PLATFORM_RASPBERRYPI2B),y)
VENDOR_BOARD_INCLUDE := $(LITEOSTOPDIR)/../../vendor/broadcom/BCM2836/board
```
...
设置HDF的顶层目录，在38行左右，有点没搞懂，为啥不直接使用一个默认的路径
